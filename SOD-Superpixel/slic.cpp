		...
		for( int k = 0; k < width; k++ )
		{
			...
			for( int i = 0; i < 8; i++ )
			{
				...
				if( (x >= 0 && x < width) && (y >= 0 && y < height) )
				{
					...
					// if(mainindex == 0)
					// printf("\nx is %d and y is %d\n", x, y);
					// 	printf("index is %d and mainindex is %d\n", index, mainindex);
...
cv::Mat SLIC::GenerateSuperpixels(cv::Mat& img, UINT numSuperpixels)
{
	if (img.empty()) {
		exit(-1);
	}
